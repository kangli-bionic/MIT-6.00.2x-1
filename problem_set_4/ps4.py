...
def r_squared(y, estimated):
    ...
    for i in range(len(y)):
        err2 = (y[i]-estimated[i])**2
        var2 = (y[i]-np.mean(y))**2
        err+=err2
        var+=var2
...
def evaluate_models_on_training(x, y, models):
    ...
    Returns:
        None
    """
    pylab.plot(x, y, 'bo', label='Measured tempreture')
    pylab.title('Tempreture Change over Years')
    pylab.xlabel('Year')
    pylab.ylabel('Tempreture (celcius)')
    for model in models:
        estimated = np.polyval(model,x)
        r2 = r_squared(y,estimated)
        pylab.plot(x, estimated, label=str(len(model)-1)+' degree Polynomial Estimated Tempreture'+
        '\n R2:'+'{0:.3f}'.format(r2))
...
### Begining of program
raw_data = Climate(
    'C:/Users/kazIm/Desktop/Python/MIT 6.00.2x/problem_set_4/data.csv')
